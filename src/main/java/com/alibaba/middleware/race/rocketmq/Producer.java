...
public class Producer {
	...
	public static void main(String[] args) throws MQClientException,
		...
		long timeEarliest = RaceUtils.getMinute(System.currentTimeMillis());
		long timeLatest = 0;
		for (int i = 0; i < count; i++) {
			try {
				...
				// System.out.println("produce order time "
				// + RaceUtils.getMinute(time));

				orderMessage.setCreateTime(time);
				...
				for (final PaymentMessage paymentMessage : paymentMessages) {
					...
					if (retVal > 0) {
						...
						if (i == count - 1) {
							timeLatest = RaceUtils.getMinute(paymentMessage
									.getCreateTime());
						}
		...
		System.err.println("earliest time : " + timeEarliest);
		System.err.println("latest time : " + timeLatest);
		producer.shutdown();
	}
}
